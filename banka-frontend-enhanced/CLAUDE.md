...
C5 (Komunikacija između banaka) frontend implemented:

**Inter-bank payments** (`routes/_authed/banking/placanja.tsx`):
- Real-time detection of foreign bank accounts from the 3-digit routing prefix
  (first 3 digits of the 18-digit account number; our prefix set via `VITE_BANK_ROUTING_PREFIX`, default `265`).
- "Međubankarski transfer" badge + bank name shown below the account number field
  when the recipient belongs to another bank.
- Backend routes the payment transparently via 2-phase commit on `POST /v1/payments`;
  the frontend calls the same endpoint — no UI change needed for the actual submission.
- `src/lib/interbank.ts` — `isInterbankAccount()`, `getBankPrefix()`, `bankNameFromPrefix()`.

**Inter-bank OTC discovery** (`components/trading/OTCDiscovery.tsx`):
- Unified discovery board shows BOTH local and external-bank public holdings.
- Local: `GET /v1/otc/discovery` (unchanged).
- External: `GET /v1/otc/external-discovery` (falls back to `/v1/otc/discovery?external=true`
  if the dedicated endpoint isn't deployed). Refetch every 30 s; `retry: 1` so a
  404 from an older backend doesn't break the local board.
- Bank-source badge per row: "Naša banka" (blue) vs. external bank name (yellow).
- "Napravi ponudu" on an external row opens `ExternalOTCOfferDialog` which calls
  `POST /v1/otc/external-offers` with `sellerBankPrefix` + normal offer fields.
- `src/lib/api/otc.ts` — `listExternalPublicHoldings()`, `createExternalOTCOffer()`.

**Inter-bank OTC negotiation** (Celina 5 spec §Pregovaranje):
- After the offer is created, threads appear in the standard `OTCThreadsPage` /
  `OTCThreadModal`. Counter-offer / withdraw / accept endpoints are the same
  (`/v1/otc/offers/{id}/counter`, etc.) — the backend routes cross-bank internally.
- No separate UI needed beyond the offer creation dialog.

**Regenerate API client** after `make proto` in backend:
```
npm run api:gen
```
The generated models will then include any new fields (e.g. `sellerBankPrefix` on
`v1PublicHoldingItem`, inter-bank fields on `v1PaymentResult`). Hand-written
wrappers in `src/lib/api/otc.ts` already use typed `ExternalPublicHoldingItem`
which can be replaced by the generated model once it exists.
