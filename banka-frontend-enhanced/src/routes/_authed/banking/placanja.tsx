...
import { isInterbankAccount, getBankPrefix, bankNameFromPrefix } from '@/lib/interbank'
...
function NewPayment() {
  ...
  const create = useMutation({
    ...
    onError: (err) => {
      toast.error(apiError(err, 'Greška pri izvršavanju plaćanja.'))
    },
  })

  // Detect inter-bank payments in real time
  const toAccountNumber = form.watch('toAccountNumber')
  const isInterbank = isInterbankAccount(normalizeAccountNumber(toAccountNumber ?? ''))
  const targetBankPrefix = getBankPrefix(normalizeAccountNumber(toAccountNumber ?? ''))
  const targetBankName = targetBankPrefix ? bankNameFromPrefix(targetBankPrefix) : null
    ...
    <main className="container max-w-3xl space-y-6 py-8">
      ...
      <form onSubmit={onSubmit} className="space-y-4 rounded-lg border border-border bg-surface p-6">
        ...
        <div>
          ...
          {isInterbank && targetBankName && (
            <div className="mt-1.5 flex items-center gap-2">
              <Badge tone="yellow">Međubankarski transfer</Badge>
              <span className="text-xs text-muted-foreground">
                Primalac je klijent banke: <strong>{targetBankName}</strong>
              </span>
            </div>
          )}
        </div>
        ...
        <Checkbox label="Sačuvaj primaoca" {...form.register('saveRecipient')} />
