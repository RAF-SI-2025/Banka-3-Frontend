...
describe('otc wrappers', () => {
  ...
  it('listExternalPublicHoldings GETs /v1/otc/external-discovery with ticker', async () => {
    let url: string | undefined
    let params: Record<string, unknown> | undefined
    mock.onGet(/\/v1\/otc\/external-discovery(?:\?.*)?$/).reply((cfg) => {
      url = cfg.url
      params = cfg.params as Record<string, unknown>
      return [200, { items: [{ holdingId: 'h-1' }] }]
    })
    const res = await listExternalPublicHoldings({ ticker: 'AAPL' })
    expect(url).toContain('/v1/otc/external-discovery')
    expect(params).toMatchObject({ ticker: 'AAPL' })
    expect(res.items?.[0]?.holdingId).toBe('h-1')
  })

  it('listExternalPublicHoldings falls back to /v1/otc/discovery?external=true on 404', async () => {
    mock.onGet(/\/v1\/otc\/external-discovery(?:\?.*)?$/).reply(404)
    let fallbackUrl: string | undefined
    let fallbackParams: Record<string, unknown> | undefined
    mock.onGet(/\/v1\/otc\/discovery(?:\?.*)?$/).reply((cfg) => {
      fallbackUrl = cfg.url
      fallbackParams = cfg.params as Record<string, unknown>
      return [200, { items: [] }]
    })

    const res = await listExternalPublicHoldings({ ticker: 'AAPL' })
    expect(fallbackUrl).toContain('/v1/otc/discovery')
    expect(fallbackParams).toMatchObject({ ticker: 'AAPL', external: true })
    expect(res.items).toEqual([])
  })

  it('createExternalOTCOffer POSTs /v1/otc/external-offers with sellerBankPrefix', async () => {
    let url: string | undefined
    let body: unknown
    mock.onPost(/\/v1\/otc\/external-offers$/).reply((cfg) => {
      url = cfg.url
      body = JSON.parse(cfg.data as string)
      return [200, { id: 'o-2', threadId: 't-2' }]
    })
    await createExternalOTCOffer({
      sellerHoldingId: 'h-2',
    ...
    expect(url).toBe('/v1/otc/external-offers')
