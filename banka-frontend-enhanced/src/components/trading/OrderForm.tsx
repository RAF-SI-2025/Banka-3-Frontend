...
export function OrderForm({
  ...
  const place = useMutation({
    ...
    onSuccess: (resp) => {
      ...
      if (!resp.exchangeClosed) toast.success('Nalog je uspešno poslat.')
    },
    onError: (err) => {
      toast.error(apiError(err, 'Greška pri slanju naloga.'))
    },
  })
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          >
            ...
            <div className="flex flex-wrap gap-4 text-sm">
              <Checkbox label="AON (sve ili ništa)" {...form.register('allOrNone')} />
              {canMargin && (
                <Checkbox label="Margin" data-cy="margin-toggle" {...form.register('margin')} />
              )}
            </div>
            {canMargin && (
