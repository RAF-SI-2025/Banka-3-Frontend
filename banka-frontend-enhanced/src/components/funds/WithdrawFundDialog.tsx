...
export function WithdrawFundDialog({
  ...
  const withdraw = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.pending) {
        toast.info('Likvidacija u toku — sredstva stižu uskoro.')
        if (onPending) onPending()
      } else {
        toast.success('Povlačenje iz fonda je uspešno izvršeno.')
      }
      setShowVerify(false)
      onClose()
    },
    onError: (e) => {
      setErr(apiError(e, 'Greška prilikom povlačenja iz fonda.'))
      toast.error(apiError(e, 'Greška prilikom povlačenja iz fonda.'))
    },
  })
          ...
          <div className="space-y-3 text-sm">
            ...
            {canActAsBank && (
              <Checkbox
                ...
                label="Povuci u ime banke"
                checked={onBehalfBank}
                onChange={(e) => setOnBehalfBank((e.target as HTMLInputElement).checked)}
              />
            )}
            ...
            <Checkbox
              ...
              label="Povuci celu poziciju"
              checked={withdrawAll}
              onChange={(e) => setWithdrawAll((e.target as HTMLInputElement).checked)}
            />
