...
export function InvestFundDialog({ open, fund, onClose, defaultOnBehalfBank = false }: Props) {
  ...
  const invest = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowVerify(false)
      toast.success('Uplata u fond je uspešno izvršena.')
      onClose()
    },
    onError: (e) => {
      setErr(apiError(e, 'Greška prilikom uplate u fond.'))
      toast.error(apiError(e, 'Greška prilikom uplate u fond.'))
    },
  })
