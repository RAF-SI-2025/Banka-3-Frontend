...
const BACKEND_REPO = process.env.CYPRESS_BACKEND_REPO ?? path.resolve(__dirname, 'Banka-3-Backend-rewrite')
...
function resetBackend(): { ok: true } {
  ...
  const augmentedPath = goBin
    ...
    : (process.env.PATH ?? '')
  execFileSync('docker', ['compose', 'run', '--rm', 'tools', 'bash', 'scripts/db/seed.sh'], {
    encoding: 'utf8',
    cwd: BACKEND_REPO,
    env: {
      ...process.env,
